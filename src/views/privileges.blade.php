...
@section('content')
    ...
    <div style="width:750px;margin:0 auto ">
        ...
        @if(CRUDBooster::getCurrentMethod() != 'getProfile')
            <p>
                <a href='{{CRUDBooster::mainpath()}}'>{{cbTrans("form_back_to_list",['module'=>CRUDBooster::getCurrentModule()->name])}}</a>
            </p>
    @endif
    ...
    <!-- Box -->
        <div class="box box-primary">
            ...
            <form method='post'
                  action='{{ (@$row->id)?route("AdminPrivilegesControllerPostEditSave")."/$row->id":route("AdminPrivilegesControllerPostAddSave") }}'>
                ...
                <div class="box-body">
                    ...
                    <div class='form-group'>
                        ...
                        <div id='set_as_superadmin' class='radio inline'>
                            <label><input required {{ (@$row->is_superadmin==1)?'checked':'' }} type='radio'
                                          name='is_superadmin' value='1'/> {{cbTrans('confirmation_yes')}}</label>
                            &nbsp;&nbsp;
                            <label><input {{ (@$row->is_superadmin==0)?'checked':'' }} type='radio' name='is_superadmin'
                                          value='0'/> {{cbTrans('confirmation_no')}}</label>
                        </div>
                    ...
                    <div id='privileges_configuration' class='form-group'>
                        ...
                        <table class='table table-striped table-hover table-bordered'>
                            ...
                            @foreach($moduls as $modul)
                                <?php
                                $roles = DB::table('cms_privileges_roles')->where('id_cms_moduls',
                                    $modul->id)->where('id_cms_privileges', $row->id)->first();
                                ?>
                                <tr>
                                    <td><?php echo $no++;?></td>
                                    <td>{{$modul->name}}</td>
                    ...
                    <div class="alert alert-warning">
                        ...
                        Give an access view, it doesn't mean create a menu for its privilege, instead you have to create
                        menu it self at the menu management
                    </div>

                </div><!-- /.box-body -->
                <div class="box-footer" align="right">
                    <button type='button' onclick="location.href='{{CRUDBooster::mainpath()}}'"
                            class='btn btn-default'>{{cbTrans("button_cancel")}}</button>
                    <button type='submit' class='btn btn-primary'><i class='fa fa-save'></i> {{cbTrans("button_save")}}
                    </button>
                </div><!-- /.box-footer-->
        </div><!-- /.box -->
