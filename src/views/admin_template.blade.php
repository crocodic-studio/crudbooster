    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <h1>
            ...
            @if($button_show)
            <a href="{{ CRUDBooster::mainpath().'?'.http_build_query(Request::all()) }}" id='btn_show_data' class="btn btn-sm btn-primary" title="{{cbTrans('action_show_data')}}">
              <i class="fa fa-table"></i> {{cbTrans('action_show_data')}}
            </a>
            @endif

            @if($button_add && CRUDBooster::canCreate())                          
            <a href="{{ CRUDBooster::mainpath('add').'?return_url='.urlencode(Request::fullUrl()).'&parent_id='.g('parent_id').'&parent_field='.$parent_field }}" id='btn_add_new_data' class="btn btn-sm btn-success" title="{{cbTrans('action_add_data')}}">
          ...
          <ol class="breadcrumb">
            <li><a href="{{CRUDBooster::adminPath()}}"><i class="fa fa-dashboard"></i> {{ cbTrans('home') }}</a></li>
