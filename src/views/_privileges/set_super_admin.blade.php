...
<div id='set_as_superadmin' class='radio inline'>
    <label>
        <input required {{ ($role->is_superadmin==1)?'checked':'' }} type='radio' name='is_superadmin' value='1'/>
        {{ cbTrans('confirmation_yes')}}
    </label>
    &nbsp;&nbsp;
    <label>
        <input {{ ($role->is_superadmin==0)?'checked':'' }} type='radio' name='is_superadmin' value='0'/>
