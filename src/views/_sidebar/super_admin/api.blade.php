<li class='treeview'>
    ...
    <ul class='treeview-menu'>
        <li class="{{ (Request::is(cbConfig('ADMIN_PATH').'/api-generator/generator*')) ? 'active' : '' }}">
            <a href='{{Route("AdminApiGeneratorControllerGetGenerator")}}'>{!! CB::icon('plus') !!} {{ cbTrans('Add_New_API') }}</a>
        </li>
        <li class="{{ (Request::is(cbConfig('ADMIN_PATH').'/api-generator')) ? 'active' : '' }}">
            <a href='{{Route("AdminApiGeneratorControllerGetIndex")}}'>{!! CB::icon('bars') !!} {{ cbTrans('list_API') }}</a>
        </li>
        <li class="{{ (Request::is(cbConfig('ADMIN_PATH').'/api-generator/screet-key*')) ? 'active' : '' }}">
