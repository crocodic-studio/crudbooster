<div class='form-group {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}' style='{{@$form["style"]}}'>
			    ...
			    @if($value)
				    ...
				    @if(@$form['filemanager_type'] == 'file')
			          	@if($value) <div style='margin-top:15px'><a id='holder-{{$name}}' href='{{asset($value)}}' target='_blank' title=' {{cbTrans("button_download_file")}} {{ basename($value)}}'><i class='fa fa-download'></i> {{cbTrans("button_download_file")}}  {{ basename($value)}}</a>
			          	&nbsp;<a class='btn btn-danger btn-delete btn-xs' onclick='swal({   title: "{{cbTrans("delete_title_confirm")}}",   text: "{{cbTrans("delete_description_confirm")}}",   type: "warning",   showCancelButton: true,   confirmButtonColor: "#DD6B55",   confirmButtonText: "{{cbTrans("confirmation_yes")}}",cancelButtonText: "{{cbTrans('button_cancel')}}",   closeOnConfirm: false }, function(){  location.href="{{url($mainpath."/delete-filemanager?file=".$row->{$name}."&id=".$row->id."&column=".$name)}}" });' href='javascript:void(0)' title='{{cbTrans('text_delete')}}'><i class='fa fa-ban'></i></a>
			          	</div>@endif
			        @else
					...
					<p><a class='btn btn-danger btn-delete btn-sm' onclick='swal({   title: "{{cbTrans("delete_title_confirm")}}",   text: "{{cbTrans("delete_description_confirm")}}",   type: "warning",   showCancelButton: true,   confirmButtonColor: "#DD6B55",   confirmButtonText: "{{cbTrans("confirmation_yes")}}", cancelButtonText: "{{cbTrans('button_cancel')}}",   closeOnConfirm: false }, function(){  location.href="{{url(CRUDBooster::mainpath("update-single?table=$table&column=$name&value=&id=$id"))}}" });'><i class='fa fa-ban'></i> {{cbTrans('text_delete')}} </a></p>
					@endif
				@endif
