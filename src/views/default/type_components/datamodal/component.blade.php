<div class='form-group form-datepicker {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}'
    ...
    <div class="{{$col_width?:'col-sm-10'}}">
        ...
        <button class="btn btn-primary" onclick="showModal{{$name}}()" type="button"><i class='fa fa-search'></i> {{trans('crudbooster.datamodal_browse_data')}}</button>
                ...
                <a class="btn btn-info" href="{{cb()->adminPath()}}/{{$form['datamodal_module_path']}}" target="_blank"><i
...
@push('bottom')
    <script type="text/javascript">
        var url_{{$name}} = "{{cb()->mainpath('modal-data')}}?table={{$form['datamodal_table']}}&columns=id,{{$form['datamodal_columns']}}&name_column={{$name}}&where={{urlencode($form['datamodal_where'])}}&select_to={{ urlencode($form['datamodal_select_to']) }}&columns_name_alias={{ urlencode($form['datamodal_columns_alias']) }}";
