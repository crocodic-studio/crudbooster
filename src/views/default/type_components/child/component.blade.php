                                ...
                                <div class="panel-body child-form-area">
                                    @foreach($form['columns'] as $col)
								        ...
								        <button class="btn btn-primary" onclick="showModal{{$name_column}}()" type="button"><i
                                                    ...
                                                    @push('bottom')
                                                        <script type="text/javascript">
                                                            var url_{{$name_column}} = "{{cb()->mainpath('modal-data')}}?table={{$col['datamodal_table']}}&columns=id,{{$col['datamodal_columns']}}&name_column={{$name_column}}&where={{urlencode($col['datamodal_where'])}}&select_to={{ urlencode($col['datamodal_select_to']) }}&columns_name_alias={{urlencode($col['datamodal_columns_alias'])}}";
                                                                ...
                                                                $(function () {
                                                                    ...
                                                                    $('#{{$name.$col['parent_select']}}, input:radio[name={{$name.$col['parent_select']}}]').change(function () {
                                                                        ...
                                                                        if (fk_value != '') {
                                                                            ...
                                                                            $.get("{{cb()->mainpath('data-table')}}?table=" + table + "&label=" + label + "&fk_name=" + fk_name + "&fk_value=" + fk_value + "&datatable_where=" + encodeURI(datatableWhere), function (response) {
                                                                                if (response) {
                                                    ...
                                                    >
                                                        ...
                                                        if ($col['datatable']) {
                                                            ...
                                                            if (! $col['datatable_where']) {
                                                                $data = cb()->get($tableJoin, NULL, "$titleField ASC");
                                                            } else {
                                                                $data = cb()->get($tableJoin, $col['datatable_where'], "$titleField ASC");
                                                            }
                                                            foreach ($data as $d) {
