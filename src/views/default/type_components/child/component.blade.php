...
<div class='form-group {{$header_group_class}}' id='form-group-{{$name}}'>								
	...
	@elseif($form2['columns'])
	<div id='panel-form-{{$name}}' class="panel panel-default">
		<div class="panel-heading">
		  	<i class='fa fa-bars'></i> {{$form2['label']}}
		</div>
		<div class="panel-body">
								...
								<div class="col-sm-10">
									...
									@elseif($col['type']=='datetime')	
											<div class="input-group">  			
												...
												<input type='text' title="{{$form2['label']}}" readonly {{$required}} {{$readonly}} {!!$placeholder!!} {{$disabled}} class='form-control notfocus datetimepicker' name="{{$name_column}}" id="{{$name_column}}" value='{{$value}}'/>	
			...
			<div class="panel panel-default">
				...
				<div class="panel-body no-padding table-responsive"  style="max-height: 400px;overflow: auto;">
					...
					<tbody>
						...
						<?php 
							$columns_tbody = [];
							$data_child = DB::table($form2['table'])
							->where($form2['foreign_key'],$id);
							foreach($form2['columns'] as $i=>$c) {
								$data_child->addselect($form2['table'].'.'.$c['name']);
							...
							$data_child = $data_child->orderby($form2['table'].'.id','desc')->get();
							foreach($data_child as $d):							
						?>
						<tr>
							@foreach($form2['columns'] as $col)
						...
						@if(count($data_child)==0)
						<tr class="trNull">
							<td colspan="{{count($form2['columns'])+1}}" align="center">{{trans('crudbooster.table_data_not_found')}}</td>
						</tr>
						@endif
					</tbody>
					</table>
