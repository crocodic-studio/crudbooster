    ...
    <table id='table_dashboard' class="table table-hover table-striped table-bordered" style="margin-bottom: 0px">
        ...
        <tr class="active">
            ...
            @if($button_table_action)
                @if(cb()->isUpdate() || cb()->isDelete() || cb()->isRead())
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <form method='get' action=''>
                                ...
                                <div class='row-filter-combo row'>
                                    ...
                                    <div class='col-sm-3'>
                                        <select name='filter_column[{{$col["field_with"]}}][type]' data-type='{{$col["type_data"]}}'
                                            ...
                                            @if(in_array($col['type_data'],['string','varchar','text','char']))
                                                <option {{ (cb()->getTypeFilter($col["field_with"]) == 'like')?"selected":"" }} value='like'>{{trans("crudbooster.filter_like")}}</option> @endif
                                            @if(in_array($col['type_data'],['string','varchar','text','char']))
                                                <option {{ (cb()->getTypeFilter($col["field_with"]) == 'not like')?"selected":"" }} value='not like'>{{trans("crudbooster.filter_not_like")}}</option>@endif

                                            <option typeallow='all'
                                                    {{ (cb()->getTypeFilter($col["field_with"]) == '=')?"selected":"" }} value='='>{{trans("crudbooster.filter_equal_to")}}</option>
                                            @if(in_array($col['type_data'],['int','integer','smallint','tinyint','mediumint','bigint','double','float','decimal','time']))
                                                <option {{ (cb()->getTypeFilter($col["field_with"]) == '>=')?"selected":"" }} value='>='>{{trans("crudbooster.filter_greater_than_or_equal")}}</option>@endif
                                            @if(in_array($col['type_data'],['int','integer','smallint','tinyint','mediumint','bigint','double','float','decimal','time']))
                                                <option {{ (cb()->getTypeFilter($col["field_with"]) == '<=')?"selected":"" }} value='<='>{{trans("crudbooster.filter_less_than_or_equal")}}</option>@endif
                                            @if(in_array($col['type_data'],['int','integer','smallint','tinyint','mediumint','bigint','double','float','decimal','time']))
                                                <option {{ (cb()->getTypeFilter($col["field_with"]) == '<')?"selected":"" }} value='<'>{{trans("crudbooster.filter_less_than")}}</option>@endif
                                            @if(in_array($col['type_data'],['int','integer','smallint','tinyint','mediumint','bigint','double','float','decimal','time']))
                                                <option {{ (cb()->getTypeFilter($col["field_with"]) == '>')?"selected":"" }} value='>'>{{trans("crudbooster.filter_greater_than")}}</option>@endif
                                            <option typeallow='all'
                                                    {{ (cb()->getTypeFilter($col["field_with"]) == '!=')?"selected":"" }} value='!='>{{trans("crudbooster.filter_not_equal_to")}}</option>
                                            <option typeallow='all'
                                                    {{ (cb()->getTypeFilter($col["field_with"]) == 'in')?"selected":"" }} value='in'>{{trans("crudbooster.filter_in")}}</option>
                                            <option typeallow='all'
                                                    {{ (cb()->getTypeFilter($col["field_with"]) == 'not in')?"selected":"" }} value='not in'>{{trans("crudbooster.filter_not_in")}}</option>
                                            @if(in_array($col['type_data'],['date','time','datetime','int','integer','smallint','tinyint','mediumint','bigint','double','float','decimal','timestamp']))
                                                <option {{ (cb()->getTypeFilter($col["field_with"]) == 'between')?"selected":"" }} value='between'>{{trans("crudbooster.filter_between")}}</option>@endif
                                            <option {{ (cb()->getTypeFilter($col["field_with"]) == 'empty')?"selected":"" }} value='empty'>Empty ( or
                                                Null)
                                            </option>
                                        </select>
                                    </div><!--END COL_SM_4-->
                                    ...
                                    <div class='col-sm-5'>
                                        <input type='text' class='filter-value form-control'
                                               style="{{ (cb()->getTypeFilter($col["field_with"]) == 'between')?"display:none":"display:block"}}"
                                               disabled name='filter_column[{{$col["field_with"]}}][value]'
                                               value='{{ (!is_array(cb()->getValueFilter($col["field_with"])))?cb()->getValueFilter($col["field_with"]):"" }}'>

                                        <div class='row between-group'
                                             style="{{ (cb()->getTypeFilter($col["field_with"]) == 'between')?"display:block":"display:none" }}">
                                            <div class='col-sm-6'>
                                                <div class='input-group {{ ($col["type_data"] == "time")?"bootstrap-timepicker":"" }}'>
                                                    ...
                                                    <input
                                                            {{ (cb()->getTypeFilter($col["field_with"]) != 'between')?"disabled":"" }}
                                                            type='text'
                                                    ...
                                                    $value = cb()->getValueFilter($col["field_with"]);
                                                    echo (cb()->getTypeFilter($col["field_with"]) == 'between') ? $value[0] : "";
                                                    ?>'>
                                                </div>
                                            </div>
                                            <div class='col-sm-6'>
                                                <div class='input-group {{ ($col["type_data"] == "time")?"bootstrap-timepicker":"" }}'>
                                                    ...
                                                    <input
                                                            ...
                                                            name='filter_column[{{$col["field_with"]}}][value][]' value='<?php
                                                    $value = cb()->getValueFilter($col["field_with"]);
                                                    echo (cb()->getTypeFilter($col["field_with"]) == 'between') ? $value[1] : "";
                                                    ?>'>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class='col-sm-2'>
                                        <select class='form-control' name='filter_column[{{$col["field_with"]}}][sorting]'>
                                            ...
                                            <option {{ (cb()->getSortingFilter($col["field_with"]) == 'asc')?"selected":"" }} value='asc'>{{trans("crudbooster.filter_ascending")}}</option>
                                            <option {{ (cb()->getSortingFilter($col["field_with"]) == 'desc')?"selected":"" }} value='desc'>{{trans("crudbooster.filter_descending")}}</option>
                                        </select>
                                    </div><!--END_COL_SM_2-->
                        ...
                        {!! cb()->getUrlParameters(['filter_column','lasturl']) !!}
        ...
        <div class="modal fade" tabindex="-1" role="dialog" id='export-data'>
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form method='post' target="_blank" action='{{ cb()->mainpath("export-data?t=".time()) }}'>
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        {!! cb()->getUrlParameters() !!}
                        <div class="modal-body">
                            <div class="form-group">
