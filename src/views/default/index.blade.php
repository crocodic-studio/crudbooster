...
@section('content')
    ...
    @if(g('return_url'))
        <p><a href='{{g("return_url")}}'><i class='fa fa-chevron-circle-{{ cbTrans('left') }}'></i>
                &nbsp; {{cbTrans('form_back_to_list',['module'=>urldecode(g('label'))])}}</a></p>
    @endif
                    ...
                    <div class="selected-action" style="display:inline-block;position:relative;">
                        <button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown"
                                aria-expanded="false"><i
                                    class='fa fa-check-square-o'></i> {{cbTrans("button_selected_action")}}
                            <span class="fa fa-caret-down"></span></button>
                        <ul class="dropdown-menu">
                            @if($button_delete && CRUDBooster::canDelete())
                                <li><a href="javascript:void(0)" data-name='delete'
                                       title='{{cbTrans('action_delete_selected')}}'><i
                                                class="fa fa-trash"></i> {{cbTrans('action_delete_selected')}}</a></li>
                            @endif
                        ...
                        <div class="input-group-btn">
                            @if(Request::get('q'))
                                ...
                                <button type='button'
                                        ...
                                        title="{{cbTrans('button_reset')}}" class='btn btn-sm btn-warning'><i
