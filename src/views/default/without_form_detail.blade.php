...
foreach($forms as $index=>$form):
	...
	$file_location = base_path('vendor/crocodicstudio/crudbooster/src/views/default/type_components/'.$type.'/tableonly-component.blade.php');
	$user_location = resource_path('views/vendor/crudbooster/type_components/'.$type.'/tableonly-component.blade.php');
...
?>           
    ...
    @if(file_exists($file_location))
    	...
    	@if($containTR)
    		@include('crudbooster::default.type_components.'.$type.'.tableonly-component')
    	@else
			<tr><td>{{$form['label']}}</td><td>@include('crudbooster::default.type_components.'.$type.'.tableonly-component')</td></tr>		
		@endif
	@elseif(file_exists($user_location))
		<?php $containTR = (substr(trim(file_get_contents($user_location)), 0, 4)=='<tr>')?TRUE:FALSE;?>
		@if($containTR)
    		@include('vendor.crudbooster.type_components.'.$type.'.tableonly-component')
    	@else
			<tr><td>{{$form['label']}}</td><td>@include('vendor.crudbooster.type_components.'.$type.'.tableonly-component')</td></tr>		
		@endif		
	@else
