...
Route::group(['middleware'=>['api','Crocodicstudio\\Crudbooster\\Http\\Middleware\\CBAuthAPI'],'namespace'=>'App\Http\Controllers'], function () {
...
Route::group(['middleware'=>['web','Crocodicstudio\Crudbooster\Http\Middleware\CBBackend'],'prefix'=>config('crudbooster.ADMIN_PATH'),'namespace'=>'App\Http\Controllers'], function () {
...
Route::group(['middleware'=>['web','Crocodicstudio\Crudbooster\Http\Middleware\CBBackend'],'prefix'=>config('crudbooster.ADMIN_PATH'),'namespace'=>$namespace], function () {

	/* DO NOT EDIT THESE BELLOW LINES */
	CRUDBooster::routeController('/','AdminController',$namespace='Crocodicstudio\Crudbooster\Http\Controllers');
	CRUDBooster::routeController('api_generator','ApiCustomController',$namespace='Crocodicstudio\Crudbooster\Http\Controllers');

	try{

		$master_controller = glob(__DIR__.'/Http/Controllers/*.php');
		...
		foreach($moduls as $v) {
			if(@$v->path && @$v->controller) {
				CRUDBooster::routeController($v->path,$v->controller,$namespace='Crocodicstudio\Crudbooster\Http\Controllers');
			}
		}
	}catch(Exception $e) {
