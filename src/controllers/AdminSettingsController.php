...
class AdminSettingsController extends CBController
{
    ...
    {
        $this->return_url = cb()->mainpath("show")."?group=".$posdata['group_setting'];
    }

    function getDeleteFileSetting()
    {
        $id = g('id');
        $row = cb()->first('cms_settings', $id);
    ...
    function postSaveSetting()
    {
        ...
        foreach ($setting as $set) {
            ...
            if (Request::hasFile($name)) {
                ...
                if ($set->content_input_type == 'upload_image') {
                    try {
                        cb()->validation([$name => 'image|max:10000']);
                    ...
                    try {
                        cb()->validation([$name => 'mimes:doc,docx,xls,xlsx,ppt,pptx,pdf,zip,rar|max:20000']);
                    } catch (CBValidationException $e) {
                        return redirect_back($e->getMessage(),'warning');
                    }
                }
        ...
        return redirect()->back()->with(['message' => 'Your setting has been saved !', 'type' => 'success']);
    }

    function hook_before_add(&$arr)
    {
        $arr['name'] = str_slug($arr['label'], '_');
        $this->return_url = cb()->mainpath("show")."?group=".$arr['group_setting'];
    }
