...
class AdminModulesController extends CBController {
	...
	public function cbInit() {		
		...
		$this->form[] = array("label"=>"Table Name","name"=>"table_name","type"=>"select2_dataenum","options"=>['enum'=>$tables_list],'required'=>true);	
	...
	public function postStep3() {
		...
		foreach($label as $i=>$lab) {
			...
			$colKey[] = '"label"=>"'.$lab.'"';
			$colKey[] = '"name"=>"'.$name[$i].'"';
			if($isImage[$i]) $colKey[] = '"image"=>true';
			if($isDownload[$i]) $colKey[] = '"download"=>true';
			if($callback[$i]) $colKey[] = '"callback"=>function($row) {'.$callback[$i].'}';
			if($width[$i]) $colKey[] = '"width"=>"'.$width[$i].'"';				
	...
	public function postStepFinish() {
		...
		return redirect()->route('AdminModulesControllerGetIndex')->with(['message'=>trans('crudbooster.alert_update_data_success'),'message_type'=>'success']);
	}
