...
{		
	...
	{
		$menu = DB::table('cms_menus')->where('id_cms_privileges',self::myPrivilegeId())
	...
	{
		$menu_active = DB::table('cms_menus')->where('id_cms_privileges',self::myPrivilegeId())
												->where('parent_id',0)
												->where('is_active',1)
		...
		foreach ($menu_active as &$menu) {
			...
			$child = DB::table('cms_menus')->where('is_dashboard',0)
												->where('is_active',1)
	...
	{
		$controllername = str_replace(["Crocodicstudio\\Crudbooster\Http\\Controllers\\","App\Http\Controllers\\"],"",strtok(Route::currentRouteAction(),'@') );
	...
	public static function generateAPI($controller_name,$table_name,$permalink,$method_type='post')
	{
		...
		class Api'.$controller_name.'Controller extends \Crocodicstudio\Crudbooster\Http\Controllers\ApiController
		{
...
class Admin'.$controllername.' extends \Crocodicstudio\Crudbooster\Http\Controllers\CBController
{
