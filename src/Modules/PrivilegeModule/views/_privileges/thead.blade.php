<tr class='active'>
    <th width='3%'>{{trans('privilege.privileges_module_list_no')}}</th>
    <th width='60%'>{{trans('privilege.privileges_module_list_mod_names')}}</th>
    <th>&nbsp;</th>
    <th>{{ trans('privilege.privileges_module_list_view') }}</th>
    <th>{{ trans('privilege.privileges_module_list_create') }}</th>
    <th>{{ trans('privilege.privileges_module_list_read') }}</th>
    <th>{{ trans('privilege.privileges_module_list_update') }}</th>
    <th>{{ trans('privilege.privileges_module_list_delete') }}</th>
</tr>
<tr class='info'>
    <th>&nbsp;</th>
    <th>&nbsp;</th>
    <th>&nbsp;</th>
    <td align="center"><input title='Check all vertical' type='checkbox' id='is_visible'/></td>
    <td align="center"><input title='Check all vertical' type='checkbox' id='is_create'/></td>
    <td align="center"><input title='Check all vertical' type='checkbox' id='is_read'/></td>
    <td align="center"><input title='Check all vertical' type='checkbox' id='is_edit'/></td>
    <td align="center"><input title='Check all vertical' type='checkbox' id='is_delete'/></td>
</tr>
                            